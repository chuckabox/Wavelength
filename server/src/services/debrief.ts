...
1) THE HEADLINE — Lead with the clearest engagement or attention shift from the session.
...
export async function* streamDebrief(raw: DebriefRequest): AsyncGenerator<string> {
  ...
  // Event-based facts (moments) lead; speech-stat facts fill in.
  const factLines = [...analysis.factLines, ...metrics.factLines];
  const prompt = buildPrompt(req, factLines);
